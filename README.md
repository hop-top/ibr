...
**Auto-escalation ladder** (`--mode auto`): when the aria + dom text-based attempt fails, ibr escalates to `--mode visual` (screenshot + Set-of-Marks). Two failure points trigger it: the model reports it could not find the element (`"outcome": "not_found"`), or the element is found but the action throws (click/type/etc.). An instruction that legitimately needs no element — a page-level scroll, an optional action the model marks `"no_element_needed"` — is skipped as before and never spends an escalation. The visual attempt is capped at `VISUAL_MAX_ESCALATIONS` (default 3 per run; explicit `--mode visual` ignores the cap). Each escalation is logged via progress feedback. On an action failure a visual-informed healing step follows if visual also fails; on a find miss there is no element or error to heal, so a visual attempt that resolves nothing leaves the instruction skipped. This allows recovery from obstructions (modals, paywalls) the text representation cannot describe.

If the visual attempt itself errors — the vision call fails, or acting on the resolved mark throws — the error is **absorbed** so the instruction still falls through to healing, and the run frequently still succeeds. It is not silent: each absorbed error is emitted as a `visual.escalation_failed` NDJSON event and recorded on the run in `visualEscalationFailures` (empty on a clean run), so an automated consumer can tell a genuinely clean run from one that healed past a real fault. See [docs/cheatsheet-agent.md](docs/cheatsheet-agent.md#absorbed-escalation-failures-visualescalation_failed).
...
Requires Node >=22. Run once to produce `dist/ibr` and `dist/ibr-server`:
