/**
 * Unit tests for src/utils/upgrader.js
 */
...
import {
  ...
  resetUpgradeTestState,
} from '../../src/utils/upgrader.js';

const _up = createUpgrader({ binary: 'ibr', githubRepo: 'hop-top/ibr' });
const { isSnoozed, snooze } = _up;
const checkForUpdate = (current, opts) => _checkForUpdate('ibr', current, opts);

// Isolate state dir per test
const tmpDir = path.join(os.tmpdir(), `ibr-upgrade-test-${process.pid}`);
beforeEach(() => {
...
describe('checkForUpdate', () => {
  it('detects update from custom URL', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ version: '2.0.0', url: 'http://example.com/ibr', notes: 'Great' }),
    });
  ...
  it('no update when same version', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ version: '1.0.0', url: 'http://example.com/ibr' }),
    });
  ...
  it('uses cache on second call', async () => {
    ...
    global.fetch = vi.fn().mockImplementation(async () => {
      ...
      return {
        ok: true,
        json: async () => ({ version: '2.0.0', url: 'http://example.com/ibr' }),
      };
    });
