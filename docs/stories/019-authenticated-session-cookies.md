...
- As a CLI user on Windows, I can import Chromium-family cookies using the
  browser's Local State key and DPAPI under my current Windows profile.
...
- Windows Local State / DPAPI failures map to actionable Windows-specific error codes.
- DB corrupt: `CookieImportError` with `code: 'db_corrupt'`.
- Linux is supported for Chrome, Brave, Edge, and Chromium.
- Windows is supported for Chrome, Brave, Edge, and Chromium.
- Windows Chromium app-bound `v20` cookies remain explicitly unsupported.
