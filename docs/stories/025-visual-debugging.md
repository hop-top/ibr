...
- As a CLI user, I run `ibr --annotate "..."` (or `-a`) to get a screenshot
...
- As a CLI user, screenshots land in `/tmp` with predictable names
  (`ibr-annotate-step-N-<ts>.png`, `ibr-failure-step-N-<ts>.png`) so I
...
- `--annotate` / `-a`: after each `#findElements` returning ≥1 element,
  full-page PNG written to `/tmp/ibr-annotate-step-<N>-<ts>.png`.
- `ANNOTATED_SCREENSHOTS_ON_FAILURE=true`: on `#actionInstruction` failure,
  PNG written to `/tmp/ibr-failure-step-<N>-<ts>.png`; error is non-fatal.
